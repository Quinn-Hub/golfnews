...
$(document).ready(function() {
  ...
  $('nav li').click(function() {
      ...
      if (tabId === 'board-game') {
        fetchTriviaQuestions('https://opentdb.com/api.php?amount=50&category=16'); // Fetch trivia question based on the selected tab
    }
  });

  $('#board-game-tab').click(function() {
    // Your board game script goes here
    console.log('Board Game tab clicked');
    fetchTriviaQuestions('https://opentdb.com/api.php?amount=50&category=16');
});

  const triviaContainer = $('#bg-trivia-container');
  const resultContainer = $('#bg-result-container');
  ...
  function fetchTriviaQuestions() {
    const loadingGif = '<img src="img/loading_.gif" alt="Loading..." style="width: 200px; height: 200px;">';
      $.ajax({
          url: 'https://opentdb.com/api.php?amount=50&category=16',
          method: 'GET',
